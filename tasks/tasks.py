...
def send_email_reminder():
    for user in User.objects.all():
        if (timezone.now() - user.preference.preferred_time > timedelta(days=1)) or (
            timezone.now() < user.preference.preferred_time
        ):
            ...
            user.preference.preferred_time += timedelta(days=1)
