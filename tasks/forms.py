...
from tasks.models import Task, User, Preference
...
class MailPreferenceForm(ModelForm):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        for i in range(0, 4):
            self.visible_fields()[i].field.widget.attrs[
                "class"
        ...
        self.visible_fields()[4].field.widget.attrs[
            "class"
        ] = """mt-1 mb-2 px-3 py-3 bg-white border
            shadow-sm border-slate-300 placeholder-slate-400
            focus:outline-none focus:border-sky-500 focus:ring-sky-500
            block w-full rounded-lg sm:text-sm focus:ring-1 bg-gray-200"""
